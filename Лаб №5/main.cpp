...
tuple<vector<Coord>, double, vector<string>> dijkstra(
    const Graph& graph,
    const Coord& start,
    const Coord& end) {
...
tuple<map<string, Coord>, vector<EdgeItem>> read_graphml(const string& file_path) {
    ...
    for (tinyxml2::XMLElement* edge = graph->FirstChildElement("edge");
        ...
        for (tinyxml2::XMLElement* data = edge->FirstChildElement("data");
            ...
            if (strcmp(key, "d19") == 0 && data->GetText()) {
    ...
    cout << "Количество вершин " << nodes.size()
        << " ребер " << edges.size() << endl;
...
void visualize_path_with_network(
    ...
    sf::RenderWindow window(
        sf::VideoMode(figsize.x, figsize.y),
        "Кратчайший маршрут");
            ...
            for (size_t i = 0; i < path.size() - 1; i++) {
                ...
                if (!street_names.empty() &&
                    ...
                    if (!font.loadFromFile("arial.ttf")) {
                        ...
                        float midX =
                            (transform_coord(path[i]).x +
                                transform_coord(path[i + 1]).x) / 2;

                        float midY =
                            (transform_coord(path[i]).y +
...
void visualize_path_with_network_duplicate(
    const map<string, Coord>& nodes,
    const vector<EdgeItem>& edges,
    const vector<Coord>& path,
    const vector<string>& street_names = vector<string>(),
...
void visualize_only_path(
    const vector<Coord>& path,
    const sf::Vector2u& figsize = sf::Vector2u(400, 400)) {
    ...
    sf::RenderWindow window(
        sf::VideoMode(figsize.x, figsize.y),
        "Кратчайший маршрут (только путь)");
...
void save_visualization(const string& filename, int dpi = 300) {
    ...
    if (!screenshot.saveToFile(filename)) {
        std::cerr << "Не удалось сохранить изображение в файл "
            << filename << std::endl;
    }
}
...
int main() {
    ...
    map<string, Coord> nodes;
    vector<EdgeItem> edges;
    std::tie(nodes, edges) = read_graphml("voronezh.graphml");
    auto t2 = std::chrono::steady_clock::now();
    std::chrono::duration<double> load_time = t2 - t1;
    std::cout << "Время загрузки графа: " << load_time.count() << " сек" << std::endl;
    ...
    string start_street_query = "Ленинградская улица";          // Название улицы для старта (пример)
    string end_street_query = "Софьинская улица";      // Название улицы для финиша (пример)
    ...
    if (start_index == -1 || end_index == -1) {
        ...
        std::tie(path, distance, street_names) =
        ...
        if (path.empty()) {
            ...
            std::cout << "Найден путь длиной "
                << distance << " км" << std::endl;
